...
public:
	...
	MeshRender(CVoxelyze* voxelyzeInstance); //!< Initializes this mesh visualization with the specified voxelyze instance. This voxelyze pointer must remain valid for the duration of this object. @param[in] voxelyzeInstance The voxelyze instance to link this mesh object to.
	~MeshRender();
...
//	void glPrepareShape();
//	void glDrawOld(); //!< Executes openGL 3 4 drawing commands to draw this mesh in an Open GL window if USE_OPEN_GL is defined.

	void RenderCube();
	void LoadCube();
	void UpdateCube(GLint attribVert, GLint attribUV, GLint attribColor);
...
private:
	...
	void addPoint(std::vector<float>* array, float x, float y, float z, float u, float v, float r, float g, float b);
